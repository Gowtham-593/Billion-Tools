# Kadhai Shorts — Claude Context

## Project Overview
Kadhai Shorts is a Tamil short-form video streaming app UI project. The goal is to build mobile screens (phone shell mockups) from Figma designs into plain HTML/CSS files.

## Tech Stack
- **Plain HTML + CSS** (no frameworks, no build tools)
- **Google Fonts** — Roboto (primary)
- **No Tailwind**, no React, no JavaScript frameworks
- SVG icons inline (no icon libraries)

## Project Structure
```
Kadhaishorts/
├── INDEX.HTML        # Main screen — Episode List with ₹ Pricing
└── CLAUDE.md
```

## Design Source
- Figma file: `Kadhai-Shorts-Screens-MCP-Test`
- File key: `kyhllNNfmbJs8ScAaXoAIs`
- Use the Figma MCP server (`mcp__claude_ai_Figma__get_design_context`) to read screens before implementing
...
| Icon    | Inactive (outlined) | Active (filled) |
|---------|---------------------|-----------------|
